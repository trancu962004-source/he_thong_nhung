...
static esp_err_t panel_st7789_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
                                           int y_start, int x_end, int y_end,
                                           const void *color_data);
static esp_err_t panel_st7789_invert_color(esp_lcd_panel_t *panel,
                                            bool invert_color_data);
static esp_err_t panel_st7789_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                      bool mirror_y);
static esp_err_t panel_st7789_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
static esp_err_t panel_st7789_set_gap(esp_lcd_panel_t *panel, int x_gap,
                                       int y_gap);
...
esp_err_t
esp_lcd_new_panel_st7789(const esp_lcd_panel_io_handle_t io,
    ...
    ESP_GOTO_ON_FALSE(st7789, ESP_ERR_NO_MEM, err, TAG,
                      "no mem for st7789 panel");
    ...
    if (panel_dev_config->vendor_config) {
        st7789->init_cmds =
            ((st7789_vendor_config_t *)panel_dev_config->vendor_config)
                ->init_cmds;
        st7789->init_cmds_size =
            ((st7789_vendor_config_t *)panel_dev_config->vendor_config)
                ->init_cmds_size;
    }
...
static esp_err_t panel_st7789_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
    ...
    size_t len =
        (x_end - x_start) * (y_end - y_start) * st7789->fb_bits_per_pixel / 8;
...
static esp_err_t panel_st7789_disp_on_off(esp_lcd_panel_t *panel,
                                           bool on_off) {
    st7789_panel_t *st7789 = __containerof(panel, st7789_panel_t, base);
    esp_lcd_panel_io_handle_t io = st7789->io;
    int command = 0;
